...
Lighthouse 12.8.2 was run against the committed unique-module art-direction candidate on the production-mode Node application with local synthetic data. Lighthouse was invoked ephemerally and is not shipped in the application dependency graph.
...
These are genuine laboratory measurements, not estimates, but localhost removes internet, Azure quota/cold-start, TLS edge and geographic latency. The Homepage and Partners LCP values are close to the 2.5 s target under synthetic mobile throttling. Re-run from the approved hosted validation environment and collect field Core Web Vitals before asserting production performance.
