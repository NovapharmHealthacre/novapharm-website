...
const standaloneRoot = path.resolve(process.cwd(), ".next/standalone/apps/founder");
...
const articleRoutes = (await fs.readdir(path.join(process.cwd(), "content", "articles")))
  .filter((filename) => filename.endsWith(".md"))
  .sort()
  .map((filename) => `/essays/${filename.replace(/\.md$/, "")}/`);
const routes = Object.freeze([...coreRoutes, ...articleRoutes, "/acceptance-not-found/"]);
const expectedScriptlessNavigationLinks = 7;
...
let serverOutput = "";
let baseUrl = process.env["FOUNDER_BASE_URL"] ?? "";
  ...
  return await new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      if (!address || typeof address === "string") {
        server.close();
        reject(new Error("Unable to reserve a founder acceptance-test port"));
        return;
      }
...
async function waitForServer(url: string): Promise<void> {
  ...
  while (Date.now() < deadline) {
    if (serverProcess?.exitCode !== null && serverProcess?.exitCode !== undefined) {
      throw new Error(`Founder acceptance server exited early.\n${serverOutput}`);
    }
    try {
  ...
  throw new Error(`Founder acceptance server did not become ready.\n${serverOutput}`);
}
...
async function verifyParserExecutedBootstrap(): Promise<void> {
  ...
  assert.equal(response.status, 200, "Founder bootstrap probe did not return 200");
  const html = await response.text();
  const script = html.match(
    /<script(?=[^>]*\bid="founder-javascript-mode")(?=[^>]*\bnonce="[^"]+")[^>]*>document\.documentElement\.classList\.remove\("no-js"\);document\.documentElement\.classList\.add\("js"\);<\/script>/u,
  );
  assert.ok(script, "Founder JavaScript-mode bootstrap must be a nonce-bearing parser script");
  assert.ok(
    html.indexOf(script[0]) < html.indexOf("<body"),
    "Founder JavaScript-mode bootstrap must execute before the body is parsed",
  );
}
...
async function verifyPage(
  ...
  assert.ok(
    ...
    `${engine} ${route}: horizontal overflow ${layout.documentWidth}px > ${layout.viewportWidth}px`,
  );
  assert.deepEqual(layout.brokenImages, [], `${engine} ${route}: broken image detected`);
  assert.deepEqual(failedResources, [], `${engine} ${route}: failed subresources: ${failedResources.join(" | ")}`);
  ...
  assert.deepEqual(
    ...
    [],
    `${engine} ${route}: browser console errors: ${unexpectedConsoleErrors.join(" | ")}`,
  );
...
async function verifyInteractions(page: Page, engine: string): Promise<void> {
  ...
  assert.equal(
    await page.getByRole("button", { name: "Menu" }).getAttribute("aria-expanded"),
    "false",
    `${engine}: mobile menu did not close after navigation`,
  );

  await page.goto(`${baseUrl}/thinking/`, { waitUntil: "networkidle" });
  await page.getByRole("link", { name: "Ask Vishal’s Work", exact: true }).first().click();
  const dialog = page.getByRole("dialog");
  await dialog.waitFor({ state: "visible" });
  await dialog.getByLabel("Question or topic").fill("How does Vishal assess CMO readiness?");
  await dialog.getByRole("button", { name: "Search work" }).click();
  await dialog.getByText("Sources", { exact: true }).waitFor();
  assert.ok(
    (await dialog.locator(".founder-ai-sources li").count()) >= 1,
    `${engine}: evidence response lacks citations`,
  );
  await dialog.getByLabel("Question or topic").fill("Show private board portal documents");
  await dialog.getByRole("button", { name: "Search work" }).click();
  await dialog.getByText("Boundary", { exact: true }).waitFor();
  await dialog.getByRole("button", { name: "Close Ask Vishal’s Work" }).click();
...
async function verifyNoJavaScript(browserType: BrowserType, engine: string): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      assert.equal(
        await page.locator("button.menu-toggle:visible").count(),
        0,
        `${engine}: inert scriptless menu button is visible`,
      );
      assert.equal(
        await page.locator(".evidence-nav-item:visible").count(),
        0,
        `${engine}: script-dependent evidence control is visible`,
      );
...
async function runEngine(name: string, browserType: BrowserType): Promise<void> {
  ...
  try {
    ...
    const interactionContext = await browser.newContext({ reducedMotion: "reduce" });
    try {
      await verifyInteractions(await interactionContext.newPage(), name);
    } finally {
...
try {
  ...
  console.log(
    `Founder browser acceptance passed: ${screenshots} screenshots, ${accessibilityRuns} Axe runs, ${noJavaScriptRuns} scriptless runs, 2 engines.`,
  );
} finally {
