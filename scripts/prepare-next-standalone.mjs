...
const requiredAsset = process.argv[3];
assert.match(appName ?? "", /^[a-z][a-z0-9-]+$/, "A safe application name is required");
assert.ok(requiredAsset && !path.isAbsolute(requiredAsset) && !requiredAsset.includes(".."), "A safe public asset path is required");

const root = process.cwd();
const deploymentRoot = path.join(root, ".next", "standalone");
const standaloneRoot = path.join(deploymentRoot, "apps", appName);
...
async function secureDeploymentArtifact() {
  ...
  assert.deepEqual(forbidden, [], `Forbidden deployment artifacts: ${forbidden.map((file) => path.relative(root, file)).join(", ")}`);
...
const [server, asset, staticEntries] = await Promise.all([
  fs.stat(serverPath),
  fs.stat(path.join(standaloneRoot, "public", requiredAsset)),
  fs.readdir(path.join(standaloneRoot, ".next", "static")),
]);
assert.ok(server.isFile(), "Standalone server entry is missing");
assert.ok(asset.isFile(), "Standalone public assets are missing");
assert.ok(staticEntries.length > 0, "Standalone Next static assets are missing");
console.log(`Prepared and scanned ${appName} standalone deployment artifact (${deploymentFileCount} files).`);
