...
function privateHead(title) {
  return `<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${title}</title><link rel="icon" href="${brandLogoSvg}" type="image/svg+xml"><link rel="stylesheet" href="/assets/css/novapharm.css"></head>`;
}
...
function applicationShell({ area, page, title, body, navigation }) {
  ...
  return `<!DOCTYPE html><html lang="en-GB" data-api-base="${apiBase}">${privateHead(`${title} | NovaPharm ${area}`)}<body data-enterprise-page="${page}"><div class="portal-shell"><aside class="portal-sidebar"><a class="portal-brand" href="${root}/dashboard/" aria-label="${area} dashboard">${brandPicture({ width: 224, height: 28, eager: true })}</a><nav aria-label="${area} navigation">${links}${crossLinks}<button class="btn btn-outline" type="button" data-logout>Logout</button></nav></aside><main class="portal-main"><div class="portal-topbar"><div><span class="eyebrow">${area}</span><h1>${title}</h1></div><span class="status-pill">Signed in as <span data-user-name></span></span></div>${body}</main></div><script src="/assets/js/api-client.js" defer></script><script src="/assets/js/portal-app.js" defer></script><script src="/assets/js/enterprise-app.js" defer></script></body></html>`;
}
...
function adminPage(page, title, body) {
  ...
  return `<!DOCTYPE html><html lang="en-GB" data-api-base="${apiBase}">${privateHead(`${title} | NovaPharm Admin`)}<body><div class="portal-shell"><aside class="portal-sidebar"><a class="portal-brand" href="/admin/dashboard/" aria-label="Administrator portal dashboard">${brandPicture({ width: 224, height: 28, eager: true })}</a><nav aria-label="Admin navigation">${links}<a href="/employee/dashboard/">Employee Portal</a><button class="btn btn-outline" type="button" data-logout>Logout</button></nav></aside><main class="portal-main"><div class="portal-topbar"><div><span class="eyebrow">Administration</span><h1>${title}</h1></div><span class="status-pill">Canonical data</span></div>${body}</main></div><script src="/assets/js/api-client.js" defer></script><script src="/assets/js/portal-app.js" defer></script><script src="/assets/js/admin-app.js" defer></script></body></html>`;
}
