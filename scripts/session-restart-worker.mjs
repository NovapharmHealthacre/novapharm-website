...
await provisionAuthUsers(portalUsersFromEnvironment(process.env, { isProduction: true }));

if (phase === "create") {
  const user = await verifyCredentials(process.env.PORTAL_USERNAME, process.env.TEST_PORTAL_PASSWORD);
  if (!user) throw new Error("Test identity could not authenticate.");
  const session = await createPersistentSession(user, "board", 60 * 60 * 1000);
  writeFileSync(statePath, session.id, { mode: 0o600 });
  chmodSync(statePath, 0o600);
} else if (phase === "verify") {
  const session = await getPersistentSession(readFileSync(statePath, "utf8").trim());
