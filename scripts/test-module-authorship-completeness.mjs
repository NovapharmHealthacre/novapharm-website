...
const overlaySource = await readFile(resolve("src/core/enterprise-domain-service.mjs"), "utf8");
...
const adminSource = sliceBetween(overlaySource, "async function authoredAdminView", "export async function enterpriseModuleSnapshot");
...
for (const module of catalog) {
  ...
  else if (["command-centre", "ceo-dashboard"].includes(module.slug)) source = overlaySource;
...
console.log(JSON.stringify({ governedModules: catalog.length, authoredByArea: counts, currentGenericFallbackModules: 0 }, null, 2));
